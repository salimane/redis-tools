...
class RedisMemStats(object):
  ...
  def _pretty_print(self, result, title, percentages=False):
	...
	val = '%s%s\t(%.2f%%)' % (val_h, val_padding, (float(val) / self.total_size) * 100)
  ...
  def print_stats(self):
    ...
    self._pretty_print(self._top_n(self.prefixes), 'Heaviest Prefixes', percentages=True)
    self._pretty_print(self._top_n(self.keys), 'Heaviest Keys', percentages=True)
    self._pretty_print(self._top_n(self.dbs), 'Heaviest Dbs', percentages=True)
    self._pretty_print(self._top_n(self.types), 'Heaviest Types', percentages=True)
